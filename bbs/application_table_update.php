...
$wr_seo_title = exist_seo_title_recursive('bbs', generate_seo_title($wr_subject), $write_table, $wr_id);
    ...
    $sql = " update g5_business_propos
                set quest_division = '{$_POST['quest_division']}',
                ...
                report_val_1 = 0,
                report_val_2 = 0,
                value = 0 where idx = '{$_POST['bo_idx']}'" ;
...
if($count_del > 0 ){
    for($k=0; $k < $count_del; $k++ ){
        sql_query("delete from {$g5['board_file_table']} where bo_table = 'g5_business_propos' and wr_id = '{$_POST['bo_idx']}' and bf_no = '{$_POST['del-no'][$k]}' ");
        $count_file_num;
    }
...
{   
    if($upload[$i]['source'] != ""){
        ...
        $row2 = sql_fetch(" select count(*) as cnt from {$g5['board_file_table']} where bo_table = 'g5_business_propos' and wr_id = '{$_POST['bo_idx']}' and bf_no = '{$i}' ");
     ...
     if($_POST['file_type'][$i] == 1){
            $rater_file++;
        } else if ($_POST['file_type'][$i] == 0){
            $all_file ++;
        }
...
if ($file_upload_msg)
    ...
    alert('수정완료', G5_BBS_URL.'/board.value.php?bo_table=business&bo_idx=7');
?>
